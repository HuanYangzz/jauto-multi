          ...
          <div class="d-flex align-items-center">
              <nav aria-label="breadcrumb">
                  <ol class="breadcrumb m-0 p-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{url()->current()}}">Non-Stock Listing ( {{sizeof($list)}} )</a>
                    </li>
                  </ol>
              </nav>
          </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                <table id="table-gift" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    <thead>
                    <tr>
                        <th>Code</th>
                        <th>Name</th>
...
<div id="modal-ban" class="modal fade" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form id="form-status" method="POST" action="{{url('/inventory/nonstock/update_status')}}">
                <div class="modal-header modal-colored-header bg-danger">
                    <h4 class="modal-title" id="modal-title">De-activate non-stock</h4>
                </div>
...
$(function(){
    ...
    $("#class-filter").chosen().change(function(){
        $("#table-gift").DataTable().draw();
    });

    $("#table-gift").dataTable.ext.search.push(
        function( settings, data, dataIndex ) {
            var td_class = $("#table-gift").DataTable().row(dataIndex).data()[2];
...
function edit_price(input,id)
{
  ...
  $.post("{{url('/inventory/nonstock/edit_price')}}",{value:value,id:id}).done(function(data){
        new PNotify({nonblock: {nonblock: !0},
          text: "Data Saved",
          type: "success",
          styling: "bootstrap3",
...
{  
  ...
  $(input).data('value',$(input).val());
  $.post("{{url('/inventory/nonstock/edit_min_price')}}",{value:value,id:id}).done(function(data){
      new PNotify({nonblock: {nonblock: !0},
        text: "Data Saved",
        type: 'success',
        styling: 'bootstrap3',
...
function edit_cost(input,id)
{
  ...
  $(input).data('value',$(input).val());
  $.post("{{url('/inventory/nonstock/edit_cost')}}",{value:value,id:id}).done(function(data){
    new PNotify({nonblock: {nonblock: !0},
    text: "Data Saved",
    type: 'success',
    styling: 'bootstrap3',
