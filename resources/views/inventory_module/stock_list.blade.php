          ...
          <div class="d-flex align-items-center">
              <nav aria-label="breadcrumb">
                  <ol class="breadcrumb m-0 p-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{url()->current()}}">Stock Listing ( {{sizeof($list)}} )</a>
                    </li>
                  </ol>
              </nav>
          </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                <table id="table-accessory" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    <thead>
                    <tr>
                        <th>Code</th>
                        <th>Name</th>
                    ...
                    @foreach($list as $item)
                        <tr>
                            ...
                            <td><select id="status_{{$item->id}}" class="form-control status-input" onchange="edit_status('{{$item->id}}')"><option @if($item->status=="ACTIVE") selected @endif>ACTIVE</option><option @if($item->status=="INACTIVE") selected @endif>INACTIVE</option></select></td>
                            <td><input type="tel" class="input-edit form-control" data-value="{{number_format($item->price,2)}}" value="{{number_format($item->price,2)}}" onchange="edit_price(this,{{$item->id}})" oninput="maxLengthCheck(this)" @if($item->status=="INACTIVE") disabled @endif /></td>
                            <td><input type="tel" class="input-edit form-control" data-value="{{number_format($item->min_price,2)}}" value="{{number_format($item->min_price,2)}}" onchange="edit_min_price(this,{{$item->id}})" oninput="maxLengthCheck(this)" @if($item->status=="INACTIVE") disabled @endif /></td>
                            <td><input type="tel" class="input-edit form-control" data-value="{{number_format($item->cost,2)}}" value="{{number_format($item->cost,2)}}" onchange="edit_cost(this,{{$item->id}})" oninput="maxLengthCheck(this)" @if($item->status=="INACTIVE") disabled @endif /></td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
...
<div id="modal-ban" class="modal fade" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <form id="form-status" method="POST" action="{{url('/inventory/stock/update_status')}}">
                <div class="modal-header modal-colored-header bg-danger">
                    <h4 class="modal-title" id="modal-title">De-activate stock</h4>
                </div>
...
$(function(){
    ...
    $("#class-filter").chosen().change(function(){
        $("#table-accessory").DataTable().draw();
    });

    $("#table-accessory").dataTable.ext.search.push(
        function( settings, data, dataIndex ) {
            var td_class = $("#table-accessory").DataTable().row(dataIndex).data()[2];
...
function edit_price(input,id)
{
  ...
  $.post("{{url('/inventory/stock/edit_price')}}",{value:value,id:id}).done(function(data){
        new PNotify({nonblock: {nonblock: !0},
          text: "Data Saved",
          type: "success",
          styling: "bootstrap3",
...
{  
  ...
  $(input).data('value',$(input).val());
  $.post("{{url('/inventory/stock/edit_min_price')}}",{value:value,id:id}).done(function(data){
      new PNotify({nonblock: {nonblock: !0},
        text: "Data Saved",
        type: 'success',
        styling: 'bootstrap3',
...
function edit_cost(input,id)
{
  ...
  $(input).data('value',$(input).val());
  $.post("{{url('/inventory/stock/edit_cost')}}",{value:value,id:id}).done(function(data){
    new PNotify({nonblock: {nonblock: !0},
    text: "Data Saved",
    type: 'success',
    styling: 'bootstrap3',
